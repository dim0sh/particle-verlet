...
pub fn env_drizzle(model: &mut Model, update: Update) {
    ...
    if model.state == State::Init {
        spawn_cup(Vec2::new(0.0, 100.0),&mut model.env_objects, 17, 2.0);
        spawn_cup(Vec2::new(-50.0, 0.0),&mut model.env_objects, 25, 2.0);
        spawn_line(Vec2::new(50.0,-50.0),&mut model.env_objects, 15, 2.0);
    }
}
